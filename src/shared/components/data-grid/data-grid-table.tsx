...
function DataGridTableBase({ children }: { children: ReactNode }) {
	 ...
	 *
	 * NOTE: we no longer emit a <colgroup> nor force an explicit table width.
	 * With table-layout: fixed + widths on the first <th> row only, the browser
	 * distributes remaining horizontal space across columns when the column
	 * widths sum to less than the table width (mirrors busanc's behavior and
	 * avoids the "blank space on the right" issue).
	 */
	...
	return (
		<table
			...
			style={props.tableLayout?.columnsResizable ? columnSizeVars : undefined}
		>
			{children}
		</table>
	)
...
function DataGridTableViewport({
	...
	return (
		<div
			...
			ref={viewportRef}
			className={cn("w-full align-top", className)}
			style={style}
		>
			{children}
		</div>
...
function DataGridTableHeadRowCell<TData>({
	...
	return (
		<th
			...
			className={cn(
				"text-secondary-foreground/80 relative h-9 text-left align-middle font-normal rtl:text-right [&:has([role=checkbox])]:pe-0",
				headerCellSpacing,
...
function DataGridTableEmpty() {
	...
	return (
		<tr>
			<td colSpan={Math.max(visibleColumnCount, 1)} className="py-4">
				{content}
			</td>
		</tr>
	)
...
function DataGridTableRowSelect<TData>({ row }: { row: Row<TData> }) {
	return (
		<>
			<div
				className={cn(
					"bg-primary inset-s-0 absolute top-0 bottom-0 hidden w-[2px]",
